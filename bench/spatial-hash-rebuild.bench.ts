/**
 * Spatial-hash rebuild micro-benchmark
 *
 * Measures the per-frame rebuild cycle in isolation:
 *   clearGrid() + N × insertEntity()
 *
 * This is the hot path implicated by the stress-test heap profile, where
 * grid.entries / _entriesPrev Map churn was the top allocator.
 */
...
function measureRebuild(bodies: Body[], cellSize: number, iters: number): RunResult {
	...
	for (let i = 0; i < 20; i++) {
		clearGrid(grid);
		for (const b of bodies) insertEntity(grid, b.id, b.x, b.y, b.r, b.r);
	}
...
function main() {
	...
	console.log('Spatial-hash rebuild micro-benchmark');
